...
###We load the data set treatment_univ where we have the results of the PCA K-means with the number of universities
...
#We add the variable treat_group for people living in provinces with low quality secondary education 

#We keep only the variables of interest in our treatment_group data set
...
#Now we create the individuos data set where we keep only the individuals we are studiing: 

#Individuals that between 17 and 19 years old and have validated high school

individuos <- Enemdu[Enemdu$cohort == "17-19",]
individuos <- individuos[individuos$education > 6,]

#We do not study Galapagos or Zonas no delimitadas because we do not have data for all years

individuos <- individuos[individuos$province != "GALAPAGOS",]
individuos <- individuos[individuos$province != "ZONAS NO DELIMITADAS",]

#We keep only individuals from before 2017

individuos <- individuos[ individuos$period < 2017,]

#We include the treat_group variable in our data sets

individuos <- merge(individuos, treatment_group, by = "province")

#We create our variable policy a dummy equal to one if the individual is from after the implementation of the exam

individuos$policy <- rep(0, length(individuos$period))
individuos$policy[individuos$period > 2011]=1

#We remove the data sets we do not want to include
...
#Indeed we find that the exam has a positive and significant effect on the likelihood of being an informal employee
...
#All in all we get the same kind of results when we include the number of universities in the PCA.
#However, now the magnitude of our results is higher
