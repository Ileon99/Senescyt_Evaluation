...
###We load the data set treat_checkm where we have the results of the PCA K-means

treatment_group <- read_feather("C:/Users/ignac/OneDrive/Documentos/PHD/Senescyt_Evaluation/Data/treat_check.feather")

###We load the Enemdu data set we cleaned with the surveys from 2007 to 2019
...
###Now we will focus on doing the DiD with individuals in the cohort of treatment

###We will keep people that have completed high school between 17 - 19 years old


#We keep only the result of interest of the PCA that will be our treatment group

treatment_group <- treatment_group[, c("province","treatment_group")]

#We modify the names to have the same that in the two data sets

names(treatment_group) <- c("province","treat_group")

##We create the data set "individuos"

#This data set contains the Enemdu data of 

#People of 17 to 19 years old

individuos <- Enemdu[Enemdu$cohort == "17-19",]

#People that have completed High School

individuos <- individuos[individuos$education > 6,]

#We do not study Galapagos and Zonas no delimitadas because we do not have enough data

individuos <- individuos[individuos$province != "GALAPAGOS",]
individuos <- individuos[individuos$province != "ZONAS NO DELIMITADAS",]

#We keep data for the period we want to study from 2007 untill 2016

#After 2016 the Exam was reformed so we do not want to measure this

individuos <- individuos[ individuos$period < 2017,]

individuos <- merge(individuos, treatment_group, by = "province")

#We create the variable "policy" that is equal to one if the individual was surveyed after 2012 (the year of the first exam)

individuos$policy <- rep(0, length(individuos$period))
individuos$policy[individuos$period > 2011]=1

#We remove Enemdu and treatment group data sets because we will not use it
...
#We want to compute some descriptive statistics to see the trends of : 
...
#We create a data set where we compute the rates of these varables per year
...
#We want to study informal employment 

#We keep only employees to see rates of informal employment
...
#We prepare the data to be able to plot

df_long <- pivot_longer(desc_stat, -period, names_to = "variable", values_to = "value")

#We keep our plot in a pdf

pdf("~/PHD/Senescyt_Evaluation/Documentation/Senecyt_stats.pdf")

# Create a vector of custom labels for each variable
custom_labels <- c(
  employment = "Proportion of employed people",
  informal = "Proportion of informal employees",
  university = "Proportion of university assistance",
  migrant = "Proportion of migrants"
)

# Create separate line plots for each variable with custom labels
ggplot(df_long, aes(x = period, y = value, color = variable)) +
  geom_line() +
  labs(
    x = "Period",
    y = "Count",
  ...
  facet_wrap(~ variable, scales = "free_y", nrow = 2, labeller = labeller(variable = custom_labels)) +
  theme_minimal()

dev.off()

#We remove the data we will not use for our DID
...
remove(df_long, custom_labels)



################################ Differences in differences #############################

##We realize our DID to evaluate the effect of the policy on the university inscription
...
##We realize our DID to evaluate the effect of the policy on the probability of being employed

reg_did_ind_c2 <- lm(employment ~ policy*treat_group +
                       age + sex + indigenous + afro + rural  + formal_per_house + h_tot_labour_income_pc +
                       h_little_kids + h_kid + h_teen + h_adult + nhousehold.y + educ_house +
                       as.factor(period) + as.factor(province),
                     data = individuos)
...
#We find that the exam has a positive and significant effect on the likelihood of being an employee

##We realize our DID to evaluate the effect of the policy on the probability of being informal

reg_did_ind_c3 <- lm(informal ~ policy*treat_group +
                       age + sex + indigenous + afro + rural  + formal_per_house + h_tot_labour_income_pc +
                       h_little_kids + h_kid + h_teen + h_adult + nhousehold.y + educ_house +
                       as.factor(period) + as.factor(province),
                     data = individuos)
...
#We find that the exam has a positive and significant effect on the likelihood of being informal 

##We realize our DID to evaluate the effect of the policy on the probability of migrating
...
#We find that the exam has a positive and significant effect on the likelihood of being a migrant

##We see the results of the regression with clustered standard errors taking the vcov matrix 

#These results are robust against heteroscedasticity

stargazer(coeftest(reg_did_ind_c1, vcov=vcovHC),coeftest(reg_did_ind_c2, vcov=vcovHC), coeftest(reg_did_ind_c3, vcov=vcovHC), coeftest(reg_did_ind_c4, vcov=vcovHC), type = "text")

#Our results are robust against heteroscedasticity

#We get the code to create a latex table
...
#Now we cluster the standard errors per each household in order to avoid unseen heterogeneity between households
...
stargazer(coeftest(reg_did_ind_c1, vcov=cluster_se1),coeftest(reg_did_ind_c2, vcov=cluster_se3),coeftest(reg_did_ind_c3, vcov=cluster_se2), coeftest(reg_did_ind_c4, vcov=cluster_se4), type = "text")
...
##We create a data set keeping only data from before 2012

#The idea is to see if we realize a did bedore the policy the groups will not have different trends

PTA <- individuos[individuos$period < 2012,]

table(PTA$period)

#Now we measure as if the policy was implemented in 2008

PTA$policy <- rep(0, length(PTA$policy))
PTA$policy[PTA$period>2008]=1
table(PTA$policy)
...
PTA_reg2 <- lm(employment ~ policy*treat_group +
                 age + sex + indigenous + afro + rural  + formal_per_house + h_tot_labour_income_pc +
                 h_little_kids + h_kid + h_teen + h_adult + nhousehold.y + educ_house +
                 as.factor(period) + as.factor(province), data = PTA)

PTA_reg3 <- lm(informal ~ policy*treat_group +
                 age + sex + indigenous + afro + rural  + formal_per_house + h_tot_labour_income_pc +
                 h_little_kids + h_kid + h_teen + h_adult + nhousehold.y + educ_house +
                 as.factor(period) + as.factor(province), data = PTA)
...
match_obj <- matchit(treat_group ~ age + sex + indigenous + afro + rural  + formal_per_house + h_tot_labour_income_pc +
                       h_little_kids + h_kid + h_teen + h_adult + nhousehold.y + educ_house,
                     data = individuos, method = "nearest", distance ="glm",
                     ratio = 1,
                     replace = FALSE)

#Let's see the results of the matching
...
remove(ddm1, ddm2, ddm3,ddm4, individuos, match_obj, match_obj1,matched_data, matched_data1, prov_univ)
